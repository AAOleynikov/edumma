...
class ApproxFunc {
  public:
    ...
    double analytical_solution(double x) {
      return -14.+12384898975268864./4271290329215643.*exp(-2.*x/3.) + 7. * x / 2.;
    }
};
...
class FEM {
  public:
    ...
    {
      if (fe_count_ <= 0) throw std::invalid_argument("fe_count must be > 0");
      if (fe_type_ != 1 && fe_type_ != 3) throw std::invalid_argument("fe_type must be 1 or 3");
  ...
  private:
    std::vector<std::vector<double>> linear_rigidity_matrix () {
    ...
    std::vector<double> linear_loads_vector () {
    ...
    std::vector<std::vector<double>> cubic_rigidity_matrix () {
    ...
    std::vector<double> cubic_loads_vector () {
    ...
    void ensembling () {
      ...
      if (fe_type == 1) {
        rm = linear_rigidity_matrix();
        lv = linear_loads_vector();
        ...
        rm = cubic_rigidity_matrix();
        lv = cubic_loads_vector();
